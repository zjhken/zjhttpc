...
impl ChunkedDecoderStream {
	...
	pub(crate) fn new_with_completion_flag(
		inner: ChainedInner, completion_flag: Arc<AtomicBool>, addr: SocketAddr, is_tls: bool,
		proxy_used: Option<HttpsProxyOption>, pool: Option<ConnectionPool>,
	) -> Self {
		Self {
			inner: Some(inner),
			state: DecoderState::ReadingChunkSizeLine,
...
impl async_std::io::Read for ChunkedDecoderStream {
	fn poll_read(
		mut self: std::pin::Pin<&mut Self>, cx: &mut std::task::Context<'_>, buf: &mut [u8],
					...
					match std::pin::Pin::new(inner_stream).poll_read(cx, &mut one_byte) {
						...
						std::task::Poll::Ready(Ok(_)) => {
							...
							if self.line_buffer.ends_with(b"\r\n") {
								...
								let chunk_size = match usize::from_str_radix(line_str.trim(), 16) {
									Ok(s) => s,
									Err(e) => {
										return std::task::Poll::Ready(Err(std::io::Error::new(
						...
						std::task::Poll::Ready(Ok(_)) => {
							...
							if self.trailer_buffer.ends_with(b"\r\n") {
								if self.trailer_buffer.len() != 2 {
									let x = String::from_utf8_lossy(&self.trailer_buffer);
									return std::task::Poll::Ready(Err(std::io::Error::new(
										std::io::ErrorKind::InvalidData,
...
impl BodyFixedLengthStream {
	...
	pub(crate) fn new_with_completion_flag(
		inner: ChainedInner, content_length: usize, completion_flag: Arc<AtomicBool>,
		addr: SocketAddr, is_tls: bool, proxy_used: Option<HttpsProxyOption>,
		pool: Option<ConnectionPool>,
	) -> Self {
		Self {
			inner: Some(inner),
...
impl Response {
	pub(crate) fn new_from_parse_result(
		http_version: &str, status_code: &str, headers_vec: Vec<(String, String)>,
		stream: BoxedStream, is_tls: bool, addr: SocketAddr, proxy_used: Option<HttpsProxyOption>,
		read_body_timeout: Option<std::time::Duration>, body_prefix: &[u8],
		pool: Option<ConnectionPool>,
		...
		let http_version = match http_version {
			...
			others => {
				return Err(InvalidResponseSnafu {
					message: format!("unsupported HTTP version: {others}"),
				}
				.build());
			}
		};
		let status_code: u16 = status_code.parse().map_err(|_| {
			InvalidResponseSnafu {
				message: format!("invalid HTTP status code: {status_code}"),
			}
			.build()
		})?;
...
mod tests {
	...
	fn test_body_fixed_length_stream() {
		...
		impl async_std::io::Write for TestStream {
			fn poll_write(
				self: std::pin::Pin<&mut Self>, _cx: &mut std::task::Context<'_>, _buf: &[u8],
			) -> std::task::Poll<std::io::Result<usize>> {
				std::task::Poll::Ready(Err(std::io::Error::new(
		...
		let chain = crate::stream::ChainRead::new(crate::stream::SliceRead::new(&[]), boxed_stream);
	...
	impl async_std::io::Read for MockStream {
		fn poll_read(
			mut self: std::pin::Pin<&mut Self>, _cx: &mut std::task::Context<'_>, buf: &mut [u8],
		) -> std::task::Poll<std::io::Result<usize>> {
