...
mod tests {
	...
	fn test_parse_cookie_with_attributes() {
		let cookies =
			Cookie::parse_from_set_cookie(vec!["sessionid=abc123; Path=/; HttpOnly; Secure"]);
		assert_eq!(cookies.len(), 1);
		assert_eq!(cookies[0].name, "sessionid");
		assert_eq!(cookies[0].value, "abc123");
	}
	...
	fn test_parse_cookie_with_equals_in_value() {
		let cookies = Cookie::parse_from_set_cookie(vec!["token=abc=123=def; Secure"]);
		assert_eq!(cookies.len(), 1);
	...
	fn test_format_for_cookie_header() {
		...
		assert_eq!(
			formatted,
			"sessionid=abc123; userdata=eyJ1c2VyIjoiYWxpY2UifQ=="
		);
	}
