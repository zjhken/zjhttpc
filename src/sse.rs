...
use crate::{error::Result, stream::ReadStream};
...
fn process_line(
	line: Vec<u8>, event_type: &mut Option<String>, data_lines: &mut Vec<String>,
	last_event_id: &mut Option<String>, retry: &mut Option<u64>,
) -> Option<SseEvent> {
	if line.is_empty() {
...
mod tests {
	...
	fn mock_stream_chunked(data: &[u8], chunk_size: usize) -> ReadStream {
		...
		impl async_std::io::Read for MockChunked {
			fn poll_read(
				...
				let n = std::cmp::min(
					buf.len(),
					std::cmp::min(self.chunk_size, self.data.len() - self.pos),
				);
				buf[..n].copy_from_slice(&self.data[self.pos..self.pos + n]);
				self.pos += n;
				std::task::Poll::Ready(Ok(n))
	...
	fn split_field_value_basic() {
		assert_eq!(
			split_field_value(b"data: hello"),
			(b"data".as_slice(), b"hello".as_slice())
		);
	}
	...
	fn split_field_value_strips_one_space() {
		assert_eq!(
			split_field_value(b"data:  two"),
			(b"data".as_slice(), b" two".as_slice())
		);
	}
	...
	fn split_field_value_no_colon() {
		assert_eq!(
			split_field_value(b"data"),
			(b"data".as_slice(), b"".as_slice())
		);
	}
	...
	impl async_std::io::Read for ErroringStream {
		fn poll_read(
			self: std::pin::Pin<&mut Self>, _cx: &mut std::task::Context<'_>, _buf: &mut [u8],
		) -> std::task::Poll<std::io::Result<usize>> {
