...
impl ZJHttpClient {
	...
	pub fn set_pool_config(
		mut self, max_per_key: usize, max_total: usize, idle_timeout: Duration,
	) -> Self {
	...
	pub async fn send_body_only(
		&self, req: &mut Request, mut stream_to_write: BoxedStream, addr: SocketAddr,
	) -> Result<Response> {
		prepare_multipart_content_length(req).await?;
...
async fn wrap_target_tls(
	client: &ZJHttpClient, req: &Request, stream: BoxedStream,
) -> Result<BoxedStream> {
	let tls_config = if req.trust_store_pem.is_some() {
		Arc::new(create_tls_config(&req.trust_store_pem)?)
	} else {
...
async fn read_headers_to_resp(
	client: &ZJHttpClient, req: &mut Request, mut stream: BoxedStream, addr: SocketAddr,
) -> Result<Response> {
...
pub async fn read_until<S>(
	stream: &mut S, delimiter: &[u8], max_bytes: usize,
