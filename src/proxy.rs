...
impl HttpsProxyOption {
	pub fn new(proxy_url: impl AsRef<str>) -> Result<Self> {
		let url: Url = proxy_url.as_ref().parse().context(InvalidUrlSnafu)?;

		if url.scheme() != "http" && url.scheme() != "https" {
			return Err(ProxySnafu {
				message: "proxy URL must use http or https scheme".to_string(),
			}
			.build());
		}
		...
		let host = url.host_str().ok_or_else(|| {
			ProxySnafu {
				message: "proxy URL must have a host".to_string(),
			}
			.build()
		})?;
		let port = url
			...
			.or_else(|_| {
				...
				if host.contains("example.com") || host.contains("localhost") {
					...
					std::net::ToSocketAddrs::to_socket_addrs(&(host, port))
						...
						.ok_or_else(|| {
							DnsSnafu {
								message: "no proxy addresses found".to_string(),
							}
							.build()
						})
				}
...
impl ProxyConnector {
	...
	pub fn new_with_trust_store(
		proxy: HttpsProxyOption, trust_store: &Option<TrustStorePem>,
	) -> Result<Self> {
	...
	pub async fn connect(
		&self, target_host: &str, target_port: u16, connect_timeout: Duration,
	) -> Result<BoxedStream> {
	...
	async fn connect_http_proxy(
		&self, proxy_addr: SocketAddr, target_host: &str, target_port: u16,
		connect_timeout: Duration,
	) -> Result<BoxedStream> {
		// Create TCP stream with connect timeout
		let mut tcp_stream = match async_std::future::timeout(
			connect_timeout,
		...
		.await
		{
			...
			Ok(Err(e)) => {
				return Err(ConnectionSnafu {
					message: format!("HTTP proxy connection failed: {e}"),
				}
				.build());
			}
			Err(_) => {
		...
		tcp_stream
			...
			.map_err(|e| {
				ProxySnafu {
					message: format!("failed to send CONNECT request to proxy: {e}"),
				}
				.build()
			})?;
		tcp_stream.flush().await.map_err(|e| {
			ProxySnafu {
				message: format!("failed to flush proxy connection: {e}"),
			}
			.build()
		})?;
	...
	async fn connect_https_proxy(
		...
		// Create TCP stream with connect timeout
		let tcp_stream = match async_std::future::timeout(
			connect_timeout,
			TcpStream::connect(&proxy_addr),
		)
		.await
		{
			...
			Ok(Err(e)) => {
				return Err(ConnectionSnafu {
					message: format!("HTTPS proxy connection failed: {e}"),
				}
				.build());
			}
			Err(_) => {
		...
		let proxy_host = self.proxy.url.host_str().ok_or_else(|| {
			ProxySnafu {
				message: "proxy URL must have a host".to_string(),
			}
			.build()
		...
		let tls_stream = tls_connector
			...
			.map_err(|e| {
				TlsSnafu {
					message: format!("failed to establish TLS connection to HTTPS proxy: {e}"),
				}
				.build()
			})?;
		...
		stream
			...
			.map_err(|e| {
				ProxySnafu {
					message: format!("failed to send CONNECT request to HTTPS proxy: {e}"),
				}
				.build()
			})?;
	...
	S: async_std::io::Read + Unpin,
{
	...
	loop {
		let n = stream.read(&mut buf[filled..]).await.map_err(|e| {
			ProxySnafu {
				message: format!("failed to read proxy CONNECT response: {e}"),
			}
			.build()
		})?;
		if n == 0 {
			return Err(ProxySnafu {
				message: "proxy closed connection before responding".to_string(),
			}
			.build());
		}
		filled += n;
		...
		if filled >= 4 && buf[..filled].windows(4).any(|w| w == b"\r\n\r\n") {
			if !buf.starts_with(b"HTTP/1.1 200") && !buf.starts_with(b"HTTP/1.0 200") {
				...
				return Err(ProxySnafu {
					message: format!("proxy CONNECT failed: {}", text.trim()),
				}
				.build());
			}
			return Ok(());
...
fn create_proxy_tls_config() -> Result<ClientConfig> {
	...
	if !cert_result.errors.is_empty() && cert_result.certs.is_empty() {
		return Err(CertificateSnafu {
			message: format!("failed to load system certs: {:?}", cert_result.errors),
		}
		.build());
	}
	...
	fn test_proxy_connect_timeout() {
		...
		async_std::task::block_on(async {
			...
			let result = connector
				.connect("example.com", 80, Duration::from_secs(1))
				.await;
