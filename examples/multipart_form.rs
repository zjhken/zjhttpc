...
fn main() -> Result<()> {
	...
	let request = Request::new("POST", "https://example.com/upload")?.set_body_multipart_form(
	...
	println!(
		"Content-Type: {}",
		request.content_type.as_deref().unwrap_or("none")
	);
	println!(
		"Content-Length: {} (0 = unknown, will use chunked encoding)",
